...
whitePanels = [x for x in code.panels if code.asoColorings[code.panels.index(x)][0] == 1]
#print(whitePanels)
xVals = list({a[0] for a in code.panels})
yVals = list({a[1] for a in code.panels})
xVals.sort()
yVals.sort(reverse = True)
canvas = [[x,y, False] for x in range(min(xVals), max(xVals) + 1) for y in range(min(yVals), max(yVals) + 1)]
for pt in canvas:
